    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-[#f8fafc] relative">
      <Container>
        ...
        <div className="grid grid-cols-5 gap-1.5 sm:gap-4 lg:gap-8 relative">
          ...
          {steps.map((s, i) => (
            <motion.div key={s.title} initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.08}} className="relative z-10 text-center">
              ...
              <p className="text-[8px] sm:text-[11px] text-[#94a3b8] leading-snug hidden sm:block">{s.desc}</p>
            </motion.div>
          ))}
        </div>
      </Container>
